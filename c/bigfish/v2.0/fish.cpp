...
void fish::ToRand()
{
	...
	for(i=0;i<MAX_MOUNT;i++)
	{
		...
		if(!cpu[i].enable&&k==0)
		{
			cpu[i].pos.y=rand()%(sc.y-DIFF_Y-80)+40;
...
void fish::ToDraw(CDC *pDC)
{
	...
	if(enable)
		Draw_player(pDC);
	todrawpowerbars_in(pDC);
	todrawpowerbars_out(pDC);
}
...
void fish::ToCheck()
{
	...
	for(i=0;i<MAX_MOUNT;i++)
	{
		...
		{
			if(cpu[i].level<=player.level)//cpu was eaten!
			{
				cpu[i].enable=0;
				player.exp+=2*(cpu[i].level+1);
				if(player.exp>player.max_exp[player.level]&&(player.level<MAX_LEVEL-1))
				{
					player.level++;
					if(player.level!=MAX_LEVEL-2)
						player.exp-=player.max_exp[player.level-1];
				}
				if(player.level==MAX_LEVEL-1)
				{
					player.exp=player.max_exp[MAX_LEVEL-1];
				}
			}
		}
...
void fish::todrawpowerbars_in(CDC *pDC)
{
	CDC temp1,mask_dc;
	CBitmap bit,mask_bit;
	BITMAP bit_info;
	bit.LoadBitmap(IDB_POWER_IN);
	bit.GetBitmap(&bit_info);
	temp1.CreateCompatibleDC(pDC);
	mask_dc.CreateCompatibleDC(pDC);
	mask_bit.CreateBitmap(bit_info.bmWidth,bit_info.bmHeight,1,1,NULL);
	...
	COLORREF l_crOldBack=temp1.SetBkColor(RGB(255,255,255));
		...
		mask_dc.StretchBlt(0,0,bit_info.bmWidth,bit_info.bmHeight,&temp1,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCCOPY);
		pDC->StretchBlt(BAR_POS_X,BAR_POS_Y,BAR_SIZE_X*((float)player.exp/player.max_exp[player.level]),BAR_SIZE_Y,&temp1,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCINVERT);
		pDC->StretchBlt(BAR_POS_X,BAR_POS_Y,BAR_SIZE_X*((float)player.exp/player.max_exp[player.level]),BAR_SIZE_Y,&mask_dc,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCAND);
		pDC->StretchBlt(BAR_POS_X,BAR_POS_Y,BAR_SIZE_X*((float)player.exp/player.max_exp[player.level]),BAR_SIZE_Y,&temp1,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCINVERT);
...
void fish::todrawpowerbars_out(CDC *pDC)
{
	CDC temp1,mask_dc;
	CBitmap bit,mask_bit;
	BITMAP bit_info;
	bit.LoadBitmap(IDB_POWER_OUT);
	bit.GetBitmap(&bit_info);
	temp1.CreateCompatibleDC(pDC);
	mask_dc.CreateCompatibleDC(pDC);
	mask_bit.CreateBitmap(bit_info.bmWidth,bit_info.bmHeight,1,1,NULL);
	...
	COLORREF l_crOldBack=temp1.SetBkColor(RGB(255,255,255));
		...
		mask_dc.StretchBlt(0,0,bit_info.bmWidth,bit_info.bmHeight,&temp1,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCCOPY);
		pDC->StretchBlt(BAR_POS_X,BAR_POS_Y,BAR_SIZE_X,BAR_SIZE_Y,&temp1,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCINVERT);
		pDC->StretchBlt(BAR_POS_X,BAR_POS_Y,BAR_SIZE_X,BAR_SIZE_Y,&mask_dc,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCAND);
		pDC->StretchBlt(BAR_POS_X,BAR_POS_Y,BAR_SIZE_X,BAR_SIZE_Y,&temp1,0,0,bit_info.bmWidth,bit_info.bmHeight,SRCINVERT);
